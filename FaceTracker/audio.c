 ...
 *
 * @param new1       New Audio to add
 *
 */
static void addMusic(Audio * root, Audio * new1);
 ...
 *
 */
static void addAudio(Audio * root, Audio * new1);
...
Audio * createAudio(const char * filename, uint8_t loop, int volume)
{
    ...
    if(SDL_LoadWAV(filename, &(new1->audio), &(new1->bufferTrue), &(new1->lengthTrue)) == NULL)
    {
    ...
    new1->buffer = new1->bufferTrue;
    new1->length = new1->lengthTrue;
    (new1->audio).callback = NULL;
    (new1->audio).userdata = NULL;
...
static inline void playAudio(const char * filename, Audio * audio, uint8_t loop, int volume)
{
    ...
    if(filename != NULL)
    {
        /* Create new1 music sound with loop */
        new1 = createAudio(filename, loop, volume);
    }
    else if(audio != NULL)
    {
    ...
    if(loop == 1)
    {
        addMusic((Audio *) (gDevice->want).userdata, new1);
    }
    else
    {
        addAudio((Audio *) (gDevice->want).userdata, new1);
    }
...
static void addMusic(Audio * root, Audio * new1)
{
    ...
    while(rootNext != NULL)
    {
        ...
        if(rootNext->loop == 1 && rootNext->fade == 0)
        {
            ...
            /* Set flag to remove any queued up music in favour of new1 music */
